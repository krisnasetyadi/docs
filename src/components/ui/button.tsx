...
export const buttonVariants = cva('active:scale-95 inline-flex items-center justify-center rounded-md font-medium transition-color focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 disabled:opacity-50 dark:focus:ring-slate-400 disabled:pointer-events-none dark:focus:ring-offset-slate-900', {
    variants: {
        variant: {
...
const Button: FC<ButtonProps> = forwardRef<HTMLButtonElement , ButtonProps>(
    ...
    <button 
        ...
        disabled={isLoading}
        {...props}>
        {isLoading ? <Loader2 className="mr-2 h-4 w-4 animated-spin" /> : null }
         {children}
    </button>
)})  
